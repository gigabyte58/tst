...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s👽MJ-BRUTE👽 %s/%s 👽OK:%s 👽CP:%s 👽 %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
def crack2(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s👽MRX-ENGINE-START👽 %s/%s 👽Ok*%s 👽CP *%s 👽 %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
